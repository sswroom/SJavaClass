...
public class Blowfish extends BlockCipher
{
	...
	private static final int origS[] = {
	...
	private void encryptInt()
	{
		...
		while (i < 16)
		{
			temp =  this.s[0x000 + ( Xl >> 24) & 0xff];
			temp += this.s[0x100 + ((Xl >> 16) & 0xff)];
			temp ^= this.s[0x200 + ((Xl >> 8)  & 0xff)];
			temp += this.s[0x300 + ( Xl        & 0xff)];
			Xr = Xr ^ temp ^ this.p[++i];
	
			temp =  this.s[0x000 + ( Xr >> 24) & 0xff];
			temp += this.s[0x100 + ((Xr >> 16) & 0xff)];
			temp ^= this.s[0x200 + ((Xr >> 8)  & 0xff)];
			temp += this.s[0x300 + ( Xr        & 0xff)];
			Xl = Xl ^ temp ^ this.p[++i];
		}
	...
	private void decryptInt()
	{
		...
		while (i > 1)
		{
			temp =  this.s[0x000 + ( Xl >> 24)];
			temp += this.s[0x100 + ((Xl >> 16) & 0xff)];
			temp ^= this.s[0x200 + ((Xl >> 8)  & 0xff)];
			temp += this.s[0x300 + ( Xl        & 0xff)];
			Xr = Xr ^ temp ^ this.p[--i];
	
			temp =  this.s[0x000 + ( Xr >> 24)];
			temp += this.s[0x100 + ((Xr >> 16) & 0xff)];
			temp ^= this.s[0x200 + ((Xr >> 8)  & 0xff)];
			temp += this.s[0x300 + ( Xr        & 0xff)];
			Xl = Xl ^ temp ^ this.p[--i];
	...
	private void expandKey(@Nullable byte[] salt, @Nullable byte[] password)
	{
		...
		{
			int salt0 = ByteTool.readMInt32(salt, 0);
			int salt4 = ByteTool.readMInt32(salt, 4);
			int salt8 = ByteTool.readMInt32(salt, 8);
			int salt12 = ByteTool.readMInt32(salt, 12);
	...
	public int encryptBlock(@Nonnull byte []inBlock, int inOfst, @Nonnull byte []outBlock, int outOfst)
	{
		...
		this.encryptInt();
		ByteTool.writeMInt32(outBlock, outOfst + 0, this.xl);
		ByteTool.writeMInt32(outBlock, outOfst + 4, this.xr);
		return 8;
	}
	...
	public void eksBlowfishSetup(int cost, @Nonnull byte[] salt, @Nonnull String password)
	{
		...
		byte[] pwdBuff = Arrays.copyOf(pwdBuff1, pwdBuff1.length + 1);
	
		this.init();
		this.expandKey(salt, pwdBuff);
		i = 1 << cost;
		while (i-- > 0)
		{
			this.expandKey(null, pwdBuff);
			this.expandKey(null, salt);
		}
	}
}
