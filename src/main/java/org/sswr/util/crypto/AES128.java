...
public final class AES128 extends BlockCipher
{
	private int encRK[];
	private int decRK[];

	public AES128(@Nonnull byte []key)
	{
		super(16);
		this.encRK = new int[44];
		this.decRK = new int[44];
		this.setKey(key);
	}
	...
	public int encryptBlock(@Nonnull byte []inBlock, int inOfst, @Nonnull byte []outBlock, int outOfst)
	{
		...
		int t2;
		int t3;
		s0 = ByteTool.readMInt32(inBlock, inOfst +  0) ^ this.encRK[0];
	...
	public int decryptBlock(@Nonnull byte []inBlock, int inOfst, @Nonnull byte []outBlock, int outOfst)
	{
		...
		s0 = ByteTool.readMInt32(inBlock, inOfst +  0) ^ this.decRK[0];
