...
public class SAMLUtil {
	...
	private static boolean decryptAssertion(@Nonnull PrivateKey key, @Nonnull XMLReader reader, @Nonnull StringBuilderUTF8 sbResult)
	{
		String nodeName;
		while ((nodeName = reader.nextElementName()) != null)
		{
	...
	private static boolean decryptResponse(@Nonnull PrivateKey key, @Nonnull XMLReader reader, @Nonnull StringBuilderUTF8 sbResult)
	{
		String nodeName;
		while ((nodeName = reader.nextElementName()) != null)
		{
			...
			{
				return decryptAssertion(key, reader, sbResult);
			}
			else
			{
				reader.skipElement();
	...
	public static boolean decryptResponse(@Nullable EncodingFactory encFact, @Nonnull PrivateKey key, @Nonnull byte[] responseXML, @Nonnull StringBuilderUTF8 sbResult)
	{
		...
		if ((nodeText = reader.nextElementName()) != null)
		{
			...
			{
				return decryptResponse(key, reader, sbResult);
			}
			else
			{
