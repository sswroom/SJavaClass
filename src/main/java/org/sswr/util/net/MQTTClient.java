...
public class MQTTClient implements Runnable, ProtocolDataListener
{
	...
	public MQTTClient(InetAddress addr, int port)
	{
		this.recvRunning = false;
		...
		this.cli = new TCPClient(addr, port);
	...
	public static boolean publishMessage(InetAddress addr, int port, String username, String password, String topic, String message)
	{
		MQTTClient cli = new MQTTClient(addr, port);
		if (cli.isError())
		{
			cli.close();
			return false;
